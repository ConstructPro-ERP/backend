...
export class AllRpcExceptionFilter implements RpcExceptionFilter<RpcException> {
    ...
    const message = typeof error === 'string' ? error : (error as { message?: string }).message ?? 'Internal error';
    return throwError(() => ({ statusCode: HttpStatus.INTERNAL_SERVER_ERROR, message }));
  }
}
